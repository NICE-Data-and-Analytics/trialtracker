...
nct_data <- shiny::reactiveVal()
isrctn_data <- shiny::reactiveVal()
eu_data <- shiny::reactiveVal()
nihr_data <- shiny::reactiveVal()
...
output$live_trial_table_nct <- DT::renderDT({
  ...
  # Debounced refresh - trigger re-evaluation when refresh finishes 
  debounced_refresh()  

  # Ensure the database connection and trial ID list are available
  ...
  nct_data() |>
    dplyr::filter(NCTId %in% trial_id_df$NCT_Ids) |>
    dplyr::left_join(NCT_dates, by = "NCTId") |>
    dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE) |>
    dplyr::select(-Query_Date, -SeeAlsoLinkURL) |>
    dplyr::select(Program, Guideline.number, `Date Added`, everything()) |>
    dplyr::mutate(across(.cols = everything(), .fns = clean_UTF)) |>
    dplyr::mutate(across(
    ...
    dplyr::distinct() |>
    DT::datatable(
      options = list(bPaginate = FALSE, scrollY = "70vh"),
      filter = "top"
    )
})
output$live_trial_table_isrctn <- DT::renderDT({
  ...
  #Set requirements for function
  req(main_con, trial_id_df, isrctn_data())

  # Create 'Date Added' summary
  ISRCTN_dates <- isrctn_data() |>
    dplyr::summarise(`Date Added` = lubridate::as_date(min(Query_Date - 1L)), .by = ISRCTN_No)

  # Filter, join, and process
  isrctn_data() |>
    dplyr::filter(ISRCTN_No %in% trial_id_df$ISRCTN_Ids) |>
    dplyr::left_join(ISRCTN_dates, by = "ISRCTN_No") |>
    dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE) |>
    dplyr::select(-Query_Date) |>
    dplyr::select(Program, Guideline.number, `Date Added`, everything()) |>
    dplyr::mutate(
      across(.cols = everything(), .fns = clean_UTF),
    ...
    dplyr::select(
      ...
      `Results completed date`, `Results posted date`, `Results published date`, Comments
    ) |>
...
output$live_trial_table_nihr <- DT::renderDT({
  ...
  nihr_data() |>
    ...
    dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE) |>
    dplyr::select(-Query_Date) |>
    dplyr::left_join(NIHR_dates, by = "project_id") |>
    dplyr::select(Program, Guideline.number, `Date Added`, everything()) |>
    dplyr::mutate(across(.cols = everything(), .fns = clean_UTF)) |>
    dplyr::rename(
      Guideline = Guideline.number,
...
output$live_trial_table_eu <- DT::renderDT({
  ...
  eu_data() |>
    ...
    dplyr::left_join(EU_dates, by = "EU_Ids") |>
    dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE) |>
    dplyr::select(
      -Query_Date,
