...
```{r}
  shiny::textOutput("timestamp_text")
```
</div>
...
dplyr::inner_join(current, old, by = names(current)[2L:(ncol(current) - 1L)], keep = FALSE) |>
  ...
  dplyr::mutate(across(.cols = everything(), .fns = ~ iconv(.x, "UTF-8", "UTF-8", sub = ""))) |>
...
# Initialise timestamp and trigger
last_refreshed <- shiny::reactiveVal(Sys.time())
refresh_trigger <- shiny::reactiveVal(0)

# Create output for 'last_updated' timestamp
output$timestamp_text <- renderText({
  paste(
    "🔄 Last refreshed at:",
...
debounced_refresh <- shiny::debounce(refresh_trigger, millis = 1000)
...
render_live_trial_table <- function(data, id_col, ids, rename_list, reorder_cols = NULL, extra_mutate = NULL, extra_filter = NULL) {
  data_filtered <- data
  if (!is.null(extra_filter)) data_filtered <- extra_filter(data_filtered)
  ...
  data_filtered |>
    ...
    dplyr::select(-Query_Date) |>
    dplyr::mutate(across(.cols = everything(), ~ iconv(.x, "UTF-8", "UTF-8", sub = ""))) |>
    { if (!is.null(extra_mutate)) extra_mutate(.) else . } |>
    dplyr::rename(!!!rename_list) |>
    { if (!is.null(reorder_cols)) dplyr::select(., all_of(reorder_cols)) else . } |>
...
output$nct_monthly_change <- DT::renderDT({
  ...
  compareDF::compare_df(
    ...
    dplyr::mutate(across(everything(), ~ iconv(.x, "UTF-8", "UTF-8", sub = ""))) |>
...
output$ISRCTN_monthly_change <- DT::renderDT({
  ...
  compareDF::compare_df(
    ...
    dplyr::mutate(
      ...
      across(everything(), ~ iconv(.x, "UTF-8", "UTF-8", sub = "")),
      Results_date_completed = lubridate::dmy(Results_date_completed),
      Results_date_posted = lubridate::dmy(Results_date_posted),
      Results_date_first_publication = lubridate::dmy(Results_date_first_publication)
    ) |>
...
```{r ServerRecentStatusChangesOLD, context = "server"}

# output$nctmonthlychange <- DT::renderDT({
#   debounced_refresh()
#   req(main_con, trial_id_df, nct_data())
# 
#   new_df <- nct_data() |>
...
#     group_col = c("Guideline.number", "NCTId"),
#     exclude = c(
#       "Query_Date", "Comments", "Condition", "BriefTitle", "Acronym",
#       "PrimaryCompletionDate", "ResultsFirstSubmitDate", "SeeAlsoLinkURL"
#     ),
#     stop_on_error = FALSE
#   )$comparison_df |>
#     dplyr::left_join(
#       new_df |>
#         dplyr::select(
#           BriefTitle, Program, Guideline.number, URL, NCTId, Condition,
#           ResultsFirstSubmitDate, Acronym, Comments
#         ),
#       by = c("Program", "Guideline.number", "URL", "NCTId")
#     ) |>
#     dplyr::mutate(
...
#     dplyr::mutate(across(.cols = everything(), .fns = ~ iconv(.x, "UTF-8", "UTF-8", sub = ""))) |>
#     dplyr::mutate(across(
#       c(CompletionDate:LastUpdatePostDate),
...
#     DT::datatable(options = list(bPaginate = FALSE, scrollY = TRUE), filter = "top")
# })
# output$ISRCTNmonthlychange <- DT::renderDT({
#   debounced_refresh()
#   req(main_con, trial_id_df, isrctn_data())
# 
#   new_df <- isrctn_data() |>
...
#     dplyr::mutate(
#       Title = stringr::str_trunc(Scientific_Title, 75L),
#       across(.cols = everything(), .fns = ~ iconv(.x, "UTF-8", "UTF-8", sub = "")),
#       Results_date_completed = lubridate::dmy(Results_date_completed),
#       Results_date_posted = lubridate::dmy(Results_date_posted),
#       Results_date_first_publication = lubridate::dmy(Results_date_first_publication)
#     ) |>
...
#     DT::datatable(options = list(bPaginate = FALSE, scrollY = TRUE), filter = "top")
# })
# output$nihrmonthlychange <- DT::renderDT({
# 
#   debounced_refresh()
#   req(main_con, trial_id_df, nihr_data())
# 
...
#   )$comparison_df |>
#     dplyr::left_join(new_df) |>
#     dplyr::mutate(Title = stringr::str_trunc(project_title, 90L)) |>
#     dplyr::mutate(across(.cols = everything(), .fns = ~ iconv(.x, "UTF-8", "UTF-8", sub = ""))) |>
#     dplyr::select(
#       Change = chng_type,
#       Program,
...
# 
# }, options = list(bPaginate = FALSE, scrollY = TRUE), filter = "top")
# output$clinicaltrialseumonthlychange <- DT::renderDT({
# 
#   debounced_refresh()
#   req(main_con, trial_id_df, eu_data())
# 
