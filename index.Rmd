...
# Cached loads for all tables (refresh when the SQLite file's mtime changes)
...
# (Optional, safe reads – uncomment later in Stage 6)
...
invalidate_cache <- function() { base::invisible(mtime_reader()) }  # call right after add/remove/archive

# Read once per mtime tick; reuse everywhere
raw_tables <- shiny::reactive({
  mtime_reader()
  tbls <- DBI::dbListTables(pool_con)
  list(
    NCT       = if ("NCT"       %in% tbls) DBI::dbReadTable(pool_con, "NCT") else base::data.frame(),
    ISRCTN    = if ("ISRCTN"    %in% tbls) DBI::dbReadTable(pool_con, "ISRCTN") else base::data.frame(),
    EU        = if ("EU"        %in% tbls) DBI::dbReadTable(pool_con, "EU") else base::data.frame(),
    NIHR      = if ("NIHR"      %in% tbls) DBI::dbReadTable(pool_con, "NIHR") else base::data.frame(),
...
# Small utils
fast_trunc <- function(x, n = 120L) { x <- base::as.character(x); i <- base::nchar(x) > n; x[i] <- base::paste0(base::substr(x[i],1L,n),"…"); x }
latest_by_id <- function(df, id_col, date_col = "Query_Date") {
  if (!base::nrow(df)) return(df)
  # Works even if Query_Date is integer/char; we coerce to Date safely
  df |>
    dplyr::mutate(
      !!date_col := base::suppressWarnings(base::as.Date(.data[[date_col]]))
    ) |>
...
# Standard DT options: server-side + paging (no nested scrolling)
dt_opts_server <- base::list(
  pageLength  = 50L,
  lengthMenu  = c(25L, 50L, 100L),
  deferRender = TRUE,
  searchDelay = 300L,
  dom         = "tip"
)

## OLD SETUP CHUNK FROM HERE
...
observe({
  invalidateLater(10 * 1000, session)  # every 10s; tie timer to the session
...
output$live_trial_table_nct <- DT::renderDT({
  ...
  nct <- raw_tables()[["NCT"]]
  if (!base::nrow(nct)) base::return(base::data.frame())

  # Compute latest row per NCTId once (cheap in R for first step)
  latest <- latest_by_id(nct, id_col = "NCTId", date_col = "Query_Date")

  latest |>
    dplyr::transmute(
      Program             = .data[["Program"]],
      Guideline           = .data[["Guideline.number"]],
      `Query Date`        = base::format(base::suppressWarnings(base::as.Date(.data[["Query_Date"]])), "%Y-%m-%d"),
      `NCT ID`            = .data[["NCTId"]],
      Title               = fast_trunc(.data[["BriefTitle"]], 120L),
