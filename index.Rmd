...
# ---------- Vectorised union block builder (+ / - rows per changed field) ----------
build_unions <- function(con, diff_map, id_col, label_cols_plus, meta, ctx_map) {
  id_q <- sql_ident(con, id_col)

  # Labels as vector (no commas inside)
  labels_vec <- vapply(label_cols_plus, function(col) {
    if (col == "Date Added") base::sprintf('base."%s" AS "%s"', col, col)
  ...
  # Context vectors per side
  ctx_vec <- function(side) {
  ...
  # Diff vector: all diff columns present each time; only the active src is filled, others NULL
  diff_vec <- function(side, which_src) {
  ...
  # Build one +/− block per field
  parts <- base::lapply(base::names(diff_map), function(src) {
    where_diff <- base::sprintf('(n.%s IS NOT o.%s)', sql_ident(con, src), sql_ident(con, src))

    plus_vec  <- base::c(labels_vec, meta_vec, diff_vec("n", src), ctx_vec("n"))
    minus_vec <- base::c(labels_vec, meta_vec, diff_vec("o", src), ctx_vec("o"))

    plus_sel  <- collapse_select(plus_vec)
    minus_sel <- collapse_select(minus_vec)

    glue::glue_sql(
      '
      SELECT "+" AS Change, {DBI::dbQuoteString(con, base::unname(diff_map[[src]]))} AS "Changed Field",
             {sql_raw(plus_sel)},
...
# ---------- Core diff SQL (IDs filtered; commas handled via vectors) ----------
build_diff_sql_filtered <- function(con, table, id_col, label_cols, meta, diff_map, ctx_map, cutoff_date) {
  ...
  # Added/Removed select lists use NULL for all diff columns
  diff_nulls_vec <- vapply(base::unname(diff_map), function(alias) {
    base::sprintf('NULL AS %s', sql_ident(con, alias))
  }, character(1L))

  # Rebuild label/meta/ctx vectors for added/removed
  labels_vec <- vapply(label_cols_plus, function(col) {
    if (col == "Date Added") base::sprintf('base."%s" AS "%s"', col, col)
    else                     base::sprintf('COALESCE(n."%1$s", o."%1$s") AS "%1$s"', col)
  }, character(1L))
  ...
  ctx_n_vec <- if (!base::length(ctx_map)) character(0) else
    vapply(base::names(ctx_map), function(k) base::sprintf('n."%s" AS "%s"', k, base::unname(ctx_map[[k]])), character(1L))
  ctx_o_vec <- if (!base::length(ctx_map)) character(0) else
    vapply(base::names(ctx_map), function(k) base::sprintf('o."%s" AS "%s"', k, base::unname(ctx_map[[k]])), character(1L))
