...
.qq <- function(alias, col) sprintf('%s."%s"', alias, gsub('`', '', col, fixed = TRUE))  # n."Program"
...
build_diff_sql <- function(table, id_col, label_cols, meta, diff_map, ctx_map, cutoff_date, ids_tmp) {
  # normalize raw names (no backticks); quoting happens in .qq()
  label_cols <- gsub("`", "", label_cols, fixed = TRUE)
  id_col     <- gsub("`", "", id_col,     fixed = TRUE)
  table      <- gsub("`", "", table,      fixed = TRUE)

  # Labels shown on every row (add Date Added from fs)
  label_cols_plus <- c(label_cols, "Date Added")

  coalesce_label <- function(col) {
    if (col == "Date Added") {
      sprintf('base."%s" AS "%s"', col, col)
    } else {
      sprintf('COALESCE(%s, %s) AS "%s"', .qq("n", col), .qq("o", col), col)
    }
  }
  ...
  # Entire SQL uses single quotes for string literals; identifiers are double-quoted via .qq()
  glue::glue(
"WITH
