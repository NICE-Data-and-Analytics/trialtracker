...
#Configure logging
trialtracker:::set_dashboard_log_file()

# DB initialisation
db_path_abs <- trialtracker:::resolve_db_path_in_app()
pool_con <- trialtracker:::init_pool_with_pragmas(db_path_abs, log_fun = trialtracker:::log_msg)
...
options(shiny.error = function() {
  ...
  trialtracker:::log_msg("SHINY ERROR:", msg)
...
# ---- Generic renderer for “Recent Status Changes” tables ----
...
# ---- wire outputs (loop) ----
for (out_id in names(nochange_specs)) local({
  out_id_local <- out_id
  spec_local   <- nochange_specs[[out_id_local]]

  output[[out_id_local]] <- DT::renderDT({
    db_mtime()

    args <- c(list(conn = pool_con), spec_local)
...
run_pubmed <- function(reg) {
  ...
  df |>
    dplyr::mutate(
      dplyr::across(dplyr::where(is.character), ~ trialtracker:::clean_text_pubmed((.x))),
...
# ---- wire outputs ----
for (out_id in names(archive_specs)) local({
  out_id_local <- out_id
  spec_local   <- archive_specs[[out_id_local]]
  ...
  output[[out_id_local]] <- DT::renderDT({
    ...
    archive_dt(
      pool       = pool_con,
      table      = spec_local$table,
      rename_map = spec_local$rename_map,
      drop_cols  = spec_local$drop_cols
    )
  }, server = FALSE)
})
```
