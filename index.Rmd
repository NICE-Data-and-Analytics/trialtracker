...
# DB initialisation
db_path <- file.path("inst", "extdata", "RSQLite_data", "TrialTracker-db.sqlite") # relative path
db_path_abs <- tryCatch(normalizePath(db_path, winslash="\\", mustWork=TRUE), error = function(e) NA_character_) # absolute_path
...
generate_nochange_table_sql <- function(conn,
  ...
  trial_ids_col <- switch(toupper(table_name),
  "NCT"    = "NCT_Ids",
  "ISRCTN" = "ISRCTN_Ids",
  "NIHR"   = "NIHR_Ids",
  "EU"     = "EU_Ids",
...
run_pubmed <- function(reg) {
  ...
  df |>
    dplyr::mutate(
      dplyr::across(where(is.character), ~ clean_text_pubmed((.x))),
...
archive_dt <- function(pool, table, rename_map, drop_cols = character()) {
  ...
  tryCatch({
    ...
    DBI::dbReadTable(pool, table) |>
      ...
      dplyr::mutate(
        dplyr::across(where(is.character), clean_utf8),
