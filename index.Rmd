...
# Build per-field + / - SELECT unions in a robust way
build_unions <- function(con, diff_map, id_col, labels_sql, ctx_sql, meta_sqls) {
  id_q   <- sql_ident(con, id_col)
  ctx_n  <- if (base::nzchar(ctx_sql$n)) base::paste0(",\n       ", ctx_sql$n) else ""
  ctx_o  <- if (base::nzchar(ctx_sql$o)) base::paste0(",\n       ", ctx_sql$o) else ""

  parts <- base::lapply(base::names(diff_map), function(src) {
    alias <- base::unname(diff_map[[src]])
    alias_q <- DBI::dbQuoteIdentifier(con, alias)
    src_n <- sql_col(con, "n", src)
    src_o <- sql_col(con, "o", src)

    glue::glue_sql(
      '
      SELECT "+" AS Change, {DBI::dbQuoteString(con, alias)} AS "Changed Field",
             {sql_raw(labels_sql)},
             {sql_raw(meta_sqls$url)} {sql_raw(meta_sqls$acr)} {sql_raw(meta_sqls$title)} {sql_raw(meta_sqls$cond)},
             {sql_raw(paste0(src_n, " AS ", alias_q))}
             {sql_raw(ctx_n)},
             DATE(n."Query_Date") AS "When",
             COALESCE(n.{`id_q`}, o.{`id_q`}) AS {`id_q`}
      FROM base
      WHERE ({sql_raw(src_n)} IS NOT {sql_raw(src_o)})
      ...
      SELECT "-" AS Change, {DBI::dbQuoteString(con, alias)} AS "Changed Field",
             {sql_raw(labels_sql)},
             {sql_raw(meta_sqls$url)} {sql_raw(meta_sqls$acr)} {sql_raw(meta_sqls$title)} {sql_raw(meta_sqls$cond)},
             {sql_raw(paste0(src_o, " AS ", alias_q))}
             {sql_raw(ctx_o)},
...
# ----- IDs via Trial_Ids (only identifiers; Program etc. come from registry tables) -----
ids_cte_sql <- function(table) {
  base::switch(
    table,
    "NCT" = '
      ids AS (
...
# ----- Core diff SQL (quoted; filtered; no temp tables) -----
build_diff_sql_filtered <- function(con, table, id_col, label_cols, meta, diff_map, ctx_map, cutoff_date) {
  ...
  labels_sql <- base::paste(vapply(label_cols_plus, coalesce_label, character(1L)), collapse = ",\n           ")

  meta_part <- function(src, out) {
    if (base::is.null(meta[[src]])) base::sprintf('NULL AS "%s",', out)
    else                            base::sprintf('COALESCE(n."%1$s", o."%1$s") AS "%2$s",', meta[[src]], out)
  }
  meta_sqls <- base::list(
    url   = meta_part("url",      "URL"),
    acr   = meta_part("acronym",  "Acronym"),
    title = meta_part("title",    "Title"),
    cond  = meta_part("condition","Condition")
  )

  ctx_sql <- base::list(
    n = if (!base::length(ctx_map)) "" else base::paste(vapply(base::names(ctx_map), function(k)
         base::sprintf('n."%s" AS "%s"', k, base::unname(ctx_map[[k]])), character(1L)), collapse = ",\n           "),
    o = if (!base::length(ctx_map)) "" else base::paste(vapply(base::names(ctx_map), function(k)
         base::sprintf('o."%s" AS "%s"', k, base::unname(ctx_map[[k]])), character(1L)), collapse = ",\n           ")
  )

  unions <- build_unions(con, diff_map, id_col, labels_sql, ctx_sql, meta_sqls)
...
{sql_raw(unions)}
UNION ALL
SELECT '+' AS Change, 'Added' AS \"Changed Field\", {sql_raw(labels_sql)},
       {sql_raw(meta_sqls$url)} {sql_raw(meta_sqls$acr)} {sql_raw(meta_sqls$title)} {sql_raw(meta_sqls$cond)},
       {sql_raw(base::paste(base::rep('NULL', base::length(diff_map)), collapse=', '))}{sql_raw(if (base::nzchar(ctx_sql$n)) base::paste0(', ', ctx_sql$n) else '')},
       DATE(n.\"Query_Date\") AS \"When\",
...
UNION ALL
SELECT '-' AS Change, 'Removed' AS \"Changed Field\", {sql_raw(labels_sql)},
       {sql_raw(meta_sqls$url)} {sql_raw(meta_sqls$acr)} {sql_raw(meta_sqls$title)} {sql_raw(meta_sqls$cond)},
       {sql_raw(base::paste(base::rep('NULL', base::length(diff_map)), collapse=', '))}{sql_raw(if (base::length(ctx_map)) base::paste0(', ', base::paste(base::rep('NULL', base::length(ctx_map)), collapse=', ')) else '')},
       DATE(o.\"Query_Date\") AS \"When\",
...
# ----- Configs (ids_get removed; simpler) -----
