...
# --- ID filter CTEs sourced from Trial_Ids ---
ids_cte_sql <- function(table) {
  switch(
    ...
    stop("Unsupported table in ids_cte_sql: ", table)
  )
}

# --- Quoted, filtered diff SQL (no temp tables; IDs from Trial_Ids) ---
build_diff_sql_filtered <- function(con, table, id_col, label_cols, meta, diff_map, ctx_map, cutoff_date) {
  ...
  meta_part <- function(src, out) {
    ...
    else sprintf('COALESCE(n."%1$s", o."%1$s") AS "%2$s",', meta[[src]], out)
  }
  ...
  ctx_select <- function(side = "n") {
    ...
    paste(vapply(seq_along(ctx_map), function(i) {
      sprintf('%s."%s" AS "%s"', side, names(ctx_map)[i], unname(ctx_map)[i])
  ...
  diff_select_for <- function(side, which_src) {
    paste(vapply(seq_along(diff_map), function(i) {
      ...
      if (identical(src, which_src)) sprintf('%s."%s" AS "%s"', side, src, alias)
  ...
  unions <- paste(vapply(names(diff_map), function(src) {
    ...
    paste0(
      ...
      sprintf(',\n       DATE(n."Query_Date") AS "When", COALESCE(n."%1$s", o."%1$s") AS "%1$s"', id_col), "\n",
      ...
      if (length(ctx_map)) paste0(",\n       ", ctx_select("n")) else "",
      sprintf(',\n       DATE(o."Query_Date") AS "When", COALESCE(n."%1$s", o."%1$s") AS "%1$s"', id_col), "\n",
      'FROM base WHERE (n."', src, '" IS NOT o."', src, '")'
    )
  }, character(1)), collapse = "\nUNION ALL\n")

  # join predicate for base_src
  join_base <- switch(
    table,
    "NIHR" = 'ON REPLACE(REPLACE(t."project_id","-",""),"/","") = ids."id_norm"',
    # others: USING(id_col)
    sprintf('USING("%s")', id_col)
  )
...
WITH
  ...
  n_rank AS (
    SELECT base_src.*,
           ROW_NUMBER() OVER (PARTITION BY {DBI::SQL(sprintf('\"%s\"', id_col))} ORDER BY \"Query_Date\" DESC) rn
    FROM base_src
  ),
  ...
  first_seen AS (
    SELECT {DBI::SQL(sprintf('\"%s\"', id_col))}, DATE(MIN(\"Query_Date\")) AS \"Date Added\"
    FROM base_src
    GROUP BY {DBI::SQL(sprintf('\"%s\"', id_col))}
  ),
  base AS (
    SELECT fs.\"Date Added\", n.*, o.*
    FROM n
    LEFT JOIN o  USING({DBI::SQL(sprintf('\"%s\"', id_col))})
    LEFT JOIN first_seen fs USING({DBI::SQL(sprintf('\"%s\"', id_col))})
    UNION ALL
    SELECT fs.\"Date Added\", n.*, o.*
    FROM o
    LEFT JOIN n USING({DBI::SQL(sprintf('\"%s\"', id_col))})
    LEFT JOIN first_seen fs USING({DBI::SQL(sprintf('\"%s\"', id_col))})
    WHERE n.{DBI::SQL(sprintf('\"%s\"', id_col))} IS NULL
  )
{DBI::SQL(unions)}
UNION ALL
SELECT '+' AS Change, 'Added' AS \"Changed Field\", {DBI::SQL(labels_sql)}, {DBI::SQL(meta_url)} {DBI::SQL(meta_acr)} {DBI::SQL(meta_title)} {DBI::SQL(meta_cond)} {DBI::SQL(paste(rep('NULL', length(diff_map)), collapse=', '))}{DBI::SQL(if (nzchar(ctx_select('n'))) paste0(', ', ctx_select('n')) else '')},
       DATE(n.\"Query_Date\") AS \"When\", n.{DBI::SQL(sprintf('\"%s\"', id_col))} AS {DBI::SQL(sprintf('\"%s\"', id_col))}
FROM base WHERE o.{DBI::SQL(sprintf('\"%s\"', id_col))} IS NULL
UNION ALL
SELECT '-' AS Change, 'Removed' AS \"Changed Field\", {DBI::SQL(labels_sql)}, {DBI::SQL(meta_url)} {DBI::SQL(meta_acr)} {DBI::SQL(meta_title)} {DBI::SQL(meta_cond)} {DBI::SQL(paste(rep('NULL', length(diff_map)), collapse=', '))}{DBI::SQL(if (length(ctx_map)) paste0(', ', paste(rep('NULL', length(ctx_map)), collapse=', ')) else '')},
       DATE(o.\"Query_Date\") AS \"When\", o.{DBI::SQL(sprintf('\"%s\"', id_col))} AS {DBI::SQL(sprintf('\"%s\"', id_col))}
FROM base WHERE n.{DBI::SQL(sprintf('\"%s\"', id_col))} IS NULL
ORDER BY \"When\" DESC, {DBI::SQL(sprintf('\"%s\"', id_col))};
",
...
run_diff_filtered <- function(cfg, cutoff_date) {
  sql <- build_diff_sql_filtered(pool_con, cfg$table, cfg$id_col, cfg$label_cols, cfg$meta, cfg$diff_map, cfg$ctx_map, cutoff_date)
  DBI::dbGetQuery(pool_con, sql)
}

# --- Configs: no ids_get now; labels/meta/diff per registry only ---
