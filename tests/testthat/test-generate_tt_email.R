...
test_that("generate_tt_email works correctly", {
  ...
  program <- "COVID"
  attachments <- dir(file.path(test_root, "test_data"), full.names = TRUE) |>
    stringr::str_subset("_COVID_")
  devs <- c("joe.bloggs@nice.org.uk", "jane.smith@nice.org.uk")
  users <- c("harry.stotle@nice.org.uk", "julie.useasar@nice.org.uk")

  # Create email (dev version)
  email <- generate_tt_email(
    program,
    attachments,
    dev_flag = TRUE,
    devs = devs,
  ...
  expect_equal(
    as.character(email$headers$Subject$values),
    "Trial Tracking Changes - COVID - DEV VERSION"
  )

  # Check attachments
  expect_equal(length(email$parts) - 1, length(attachments))
  expect_equal(
    email$parts[[1]]$filename,
    "EU_COVID_Registry_Changes-2024-09-04.xlsx"
  )
  expect_equal(
    email$parts[[2]]$filename,
    "ISRCTN_COVID_Registry_Changes-2024-09-04.xlsx"
  )
