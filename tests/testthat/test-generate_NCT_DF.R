...
test_that("generate_NCT_DF works correctly", {
    ...
    studies = list(
      list(
        protocolSection = list(
          ...
          referencesModule = list(
            seeAlsoLinks = list(list(url = "http://example.com"))
          )
        )
      )
    )
  ...
  expect_equal(df_empty, tibble::tibble(
    Rank = integer(),
    NCTId = character(),
  ...
  mock_response_missing_fields <- list(
    studies = list(
      list(
        protocolSection = list(
          identificationModule = list(
  ...
  expect_error(generate_NCT_DF("invalid_url"), "Invalid URL")
