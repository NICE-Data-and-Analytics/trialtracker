...
dbReadTable(con, "NCT") %>%
  ...
  datatable(options = list(bPaginate = FALSE), filter = "top")
```
...
observeEvent(input$registry_remove, {
  ...
  choices_list <- dbReadTable(con, "Trial_Ids") %>%
    select(input$registry_remove) %>%
    drop_na() %>%
    arrange(input$registry_remove)
...
observeEvent(input$button_archive, {
  ...
  pulled_trial_data <- dbGetQuery(con, exp_archive_pull()) %>%
    mutate("Comments" = as.character(input$comments_archive)) %>%
