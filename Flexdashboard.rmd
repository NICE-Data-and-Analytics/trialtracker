...
```{r CTGtable}
# create added_dates
NCT_dates <- DBI::dbReadTable(con, "NCT") |>
  dplyr::summarise("Date Added" = lubridate::as_date(min(Query_Date - 1L)), .by = NCTId)

DBI::dbReadTable(con, "NCT") |>
  dplyr::filter(NCTId %in% Trial_Ids$NCT_Ids) |>
...
```{r ISRCTNtable}
# create added_dates
ISRCTN_dates <- DBI::dbReadTable(con, "ISRCTN") |>
  dplyr::summarise("Date Added" = lubridate::as_date(min(Query_Date - 1L)), .by = ISRCTN_No)

DBI::dbReadTable(con, "ISRCTN") |>
  dplyr::filter(ISRCTN_No %in% Trial_Ids$ISRCTN_Ids) |>
...
``` {r NIHRtable}
# create added_dates
NIHR_dates <- DBI::dbReadTable(con, "NIHR") |>
...
DBI::dbReadTable(con, "NIHR") |>
  ...
  dplyr::filter(project_id_filter %in% stringr::str_replace_all(Trial_Ids$NIHR_Ids, "[^\\d]", "")) |>
...
``` {r CGTmonthlychange}
new_df <- DBI::dbReadTable(con, "NCT") |>
  dplyr::filter(NCTId %in% Trial_Ids$NCT_Ids) |>
  dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE) |>
...
old_df <- DBI::dbReadTable(con, "NCT") |>
  dplyr::filter(
    Query_Date <= Sys.Date() - 31L,
    NCTId %in% Trial_Ids$NCT_Ids
  ) |>
  dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE) |>
  dplyr::mutate(across(c(PrimaryCompletionDate:LastUpdatePostDate), \(x) lubridate::as_date(lubridate::parse_date_time(x, c("ymd", "bdy"), truncated = 1L))))
...
new_df <- DBI::dbReadTable(con, "ISRCTN") |>
  dplyr::filter(ISRCTN_No %in% Trial_Ids$ISRCTN_Ids) |>
  dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE)

old_df <- DBI::dbReadTable(con, "ISRCTN") |>
  dplyr::filter(
    Query_Date <= Sys.Date() - 31L,
    ISRCTN_No %in% Trial_Ids$ISRCTN_Ids
  ) |>
  dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE)
...
``` {r NIHRmonthlychange}
new_df <- DBI::dbReadTable(con, "NIHR") |>
  dplyr::mutate("project_id_filter" = stringr::str_replace_all(project_id, "[^\\d]", "")) |>
  dplyr::filter(project_id_filter %in% stringr::str_replace_all(Trial_Ids$NIHR_Ids, "[^\\d]", "")) |>
  dplyr::select(-project_id_filter) |>
  dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE)

old_df <- DBI::dbReadTable(con, "NIHR") |>
  dplyr::mutate("project_id_filter" = stringr::str_replace_all(project_id, "[^\\d]", "")) |>
  dplyr::filter(project_id_filter %in% stringr::str_replace_all(Trial_Ids$NIHR_Ids, "[^\\d]", "")) |>
  dplyr::select(-project_id_filter) |>
...
new_df <- DBI::dbReadTable(con, "EU") |>
  dplyr::filter(EU_Ids %in% Trial_Ids$EU_Ids) |>
  dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE)

old_df <- DBI::dbReadTable(con, "EU") |>
  dplyr::filter(
    Query_Date <= Sys.Date() - 30L,
    EU_Ids %in% Trial_Ids$EU_Ids
  ) |>
  dplyr::slice_max(Query_Date, n = 1L, with_ties = TRUE)
...
compareDF::compare_df(
  ...
  dplyr::left_join(new_df) |>
  dplyr::mutate( # `Query Date` = as.Date(Query_Date, origin = "1970-01-01"),
...
``` {r nochangeCTG}
current <- DBI::dbReadTable(con, "NCT") |>
  dplyr::filter(Query_Date == lubridate::as_date(max(Query_Date)))

old <- DBI::dbReadTable(con, "NCT") |>
...
``` {r nochangeISRCTN}
current <- DBI::dbReadTable(con, "ISRCTN") |>
  dplyr::filter(Query_Date == lubridate::as_date(max(Query_Date)))

old <- DBI::dbReadTable(con, "ISRCTN") |>
  dplyr::filter(Query_Date == (lubridate::as_date(max(Query_Date)) - months(6L)))
...
``` {r nochangeNIHR}
current <- DBI::dbReadTable(con, "NIHR") |>
  dplyr::filter(Query_Date == lubridate::as_date(max(Query_Date)))

old <- DBI::dbReadTable(con, "NIHR") |>
  dplyr::filter(Query_Date == (lubridate::as_date(max(Query_Date)) - months(6L)))
...
```{r nochangeEU}
current <- DBI::dbReadTable(con, "EU") |>
  dplyr::filter(Query_Date == lubridate::as_date(max(Query_Date)))

old <- DBI::dbReadTable(con, "EU") |>
  dplyr::filter(Query_Date == (lubridate::as_date(max(Query_Date)) - months(6L)))
...
``` {r EUpubstable}
if ("EU_PM" %in% DBI::dbListTables(con)) {
  DBI::dbReadTable(con, "EU_PM") |>
    dplyr::mutate(across(.cols = everything(), .fns = clean_UTF)) |>
    dplyr::mutate(
      Query_Date = format(as.Date(as.numeric(Query_Date), origin = "1970-01-01"), "%Y/%m/%d"),
      abstract = stringr::str_trunc(abstract, 200L)
...
shiny::observeEvent(input$uploadtemplate, {
  NCT_check <- stringr::str_detect(replace_na(to_add()$NCT_Ids, ""), NCT_pattern)
  EU_check <- stringr::str_detect(replace_na(to_add()$EU_Ids, ""), EU_pattern)
  ISRCTN_check <- stringr::str_detect(replace_na(to_add()$ISRCTN_Ids, ""), ISRCTN_pattern)
  NIHR_check <- stringr::str_detect(replace_na(to_add()$NIHR_Ids, ""), NIHR_pattern)
  Valid_ID <- {
...
shiny::observeEvent(input$registry_remove, {
  choices_list <- DBI::dbReadTable(con, "Trial_Ids") |>
...
shiny::observeEvent(input$button_archive, {
  ...
  pulled_trial_data <- DBI::dbGetQuery(con, exp_archive_pull()) |>
  ...
  # write to db (append)
  DBI::dbWriteTable(con, paste0(stringr::str_sub(input$registry_remove, end = -5L), "_rxv"), pulled_trial_data, append = TRUE)
...
pulled_comments <- shiny::reactive({
  DBI::dbGetQuery(con, exp_comment_pull()) |> as.character()
})
...
shiny::observeEvent(input$button_comment, {
  ...
  # Action SQL
  DBI::dbExecute(con, update_comment_sql)
...
DBI::dbReadTable(con, "ISRCTN_rxv") |>
  dplyr::mutate(across(.cols = everything(), .fns = clean_UTF),
    `Archive Date` = lubridate::as_date(as.numeric(Query_Date))
  ) |>
  dplyr::select(-Query_Date) |>
