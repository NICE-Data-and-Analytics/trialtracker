...
# 3) Add in field for condition (short.working title) to the add trial page (or nix?)
...
dbReadTable(con, "EU") %>%
  ...
  slice_max(Query_Date, n = 1, with_ties = TRUE) %>%
  select(
    -Query_Date, -a31_title_of_the_trial_for_lay_people_in_easily_understood_ie_nontechnical_language, -EU_Ids
  ) %>%
  mutate(across(.fns = clean_UTF)) %>% 
  distinct() %>% 
  rename(
...
observeEvent(input$button_add, {
  ...
  if (input$registry_add == 'NCT_Ids') {
    source('Download_Trial_Info_NCT.R')
  } else if (input$registry_add == 'ISRCTN_Ids') {
    source('Download_Trial_Info_ISRCTN.R')
  } else if (input$registry_add == 'NIHR_Ids') {
     source('Download_Trial_Info_NIHR.R')
  } else if (input$registry_add == 'EU_Ids') {
...
observeEvent(input$button_archive, {
  ...
  pulled_trial_data <- dbGetQuery(con, exp_archive_pull()) %>%
    mutate("Archiving_Comments" = as.character(input$comments_archive)) %>%
...
inputPanel(
    ...
    selectInput('ID_comment',
                label = 'Select Trial ID reference number',
                choices = ""),
    textInput("trial_comments", label = "Comments", placeholder = "Trial Comments"),
...
observeEvent(input$ID_comment, {
  ...
  updateTextInput(session, "trial_comments", value = pulled_comments())
